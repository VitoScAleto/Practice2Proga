...
void Printf(int& number, int& degree, int& primeNumber) 
{ // вывод
	cout << "Input the data: \nnumber = ";
	cin >> number;
	do {
...
bool isValidPrimeNumber(int validPrimeNumber) 
...
void PrintThFerma(int& number, int& degree, int& primeNumber)
{ // вывод теоремы ферма когда а кратно р
		cout << "\nBy Fermat's theorem:\n" << endl;
		if (FermsTheorema(number,primeNumber) == 0) 
		{          // когда целое кратно простому
			cout << number << "^" << degree << " mod " << primeNumber << " = 0" << endl;
		}
		else 
		{    //когда целое не кратно простому
